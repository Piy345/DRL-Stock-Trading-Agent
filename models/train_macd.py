...
def train_macd_models():
    """
    Train A2C, PPO, DQN on MACD env using best hyperparams from paper.
    """
    print("Training models on MACD Environment...")
    ...
    env = DummyVecEnv([lambda: MACDTradingEnv(train_df)])
    ...
    ppo_model.save(os.path.join(results_dir, "ppo_macd"))
    ...
    dqn_model.save(os.path.join(results_dir, "dqn_macd"))
    ...
    a2c_model.save(os.path.join(results_dir, "a2c_macd"))
    
    print("Training on MACD Environment complete. Models saved to results/")
